...
const Profile = ({ data }) => {
  ...
  return (
    <>
      {/* AUDIO BLOCK */}
      <article>
        <span className="text-xl font-bold block text-black">オーディオ</span>
        <div className="mt-5 flex flex-col space-y-5">
          {audio.map((audio, index) => (
            <div
              key={index}
              className="flex flex-row space-x-4 rounded-lg overflow-hidden bg-grey-3"
            >
              ...
              <div className="pt-2 pb-1 flex flex-col">
                ...
                <span className="block text-xs text-grey">{audio.subTitle}</span>
          ...
          {links.slice(2).map((link, index) => (
            ...
            >
              ...
              <div className="flex flex-col justify-center">
                ...
                <span className="text-xs text-grey">{link.subTitle}</span>
              </div>
            </div>
          ))}
        </div>
